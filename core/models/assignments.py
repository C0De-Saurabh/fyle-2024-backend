...
class Assignment(db.Model):
    ...
    def submit(cls, _id, teacher_id, auth_principal: AuthPrincipal):
        ...
        assertions.assert_valid(assignment.content is not None, 'assignment with empty content cannot be submitted')
        assertions.assert_valid(assignment.state == AssignmentStateEnum.DRAFT,'only a draft state assignment can be submitted')
    ...
    def mark_grade_teacher(cls, _id, grade, auth_principal: AuthPrincipal):
        ...
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_found(assignment.state!=AssignmentStateEnum.DRAFT, 'DRAFT Assignment cannot be graded')
        ...
        if grade not in [grade.value for grade in GradeEnum]:
    ...
    def mark_grade_principal(cls, _id, grade, auth_principal: AuthPrincipal):
        assignment = cls.query.get(_id)
        if not assignment:
            raise ValueError("No assignment with this ID was found.")

        if assignment.state == AssignmentStateEnum.DRAFT:
            raise ValueError("Draft assignment cannot be graded.")
